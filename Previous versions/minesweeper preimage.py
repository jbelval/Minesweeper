...
class Game(tk.Frame):
    ...
    def __init__(self, root=None, size=10):
        ...
        self.flag_image = tk.PhotoImage(file = r'C:\Users\jbelv\Downloads\Flag.png')
    ...
    def check(self, x, y):
        if self.buttons[x][y].is_flagged:
            return
        if self.buttons[x][y]['bg']!='SystemButtonFace':
            return
        ...
        if type(self.buttons[x][y]).__name__ == 'Mine':
            ...
            self.buttons[x][y]['bg'] = '#DDDDDD'
            self.buttons[x][y]['fg'] = self.button_format(adj_bombs)[0]
            self.found_tiles +=1
            if adj_bombs != 0:
                self.buttons[x][y]['text'] = adj_bombs
            elif adj_bombs == 0:
                for i in range(max(0, x-1), min(self.size-1, x+1)+1):
                    for j in range(max(0, y-1), min(self.size-1, y+1)+1):
                        if type(self.buttons[x][y]).__name__ == 'Empty' and self.buttons[i][j]['bg']=='SystemButtonFace':
                            self.check(i, j)
...
class Tile(tk.Button):
    ...
    def __init__(self, game, x, y):
        super().__init__(height=1, width=2, font=("Courier", 15))
        self.game = game
        self.x = x
        self.y = y
        self['text'] = '   '
    ...
    def right_click(self, event):
        if self.clicked:
            return
        if self.is_flagged:
            self.config(image='', height=1, width=2)
            self.is_flagged = False
        else:
            self.config(image = self.game.flag_image, height=30, width=30)
